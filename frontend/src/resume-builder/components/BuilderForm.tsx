...
export const BuilderForm = ({ data, setData, currentStep, onStepChange }: BuilderFormProps) => {
  ...
  return (
    <div>
      <nav className="-mx-3 mb-6 overflow-x-auto px-3 pb-2 sm:mx-0 sm:mb-8 sm:px-0" aria-label={t("resumeBuilder.steps.label")}>
      ...
      <div className="mt-9 flex flex-wrap items-center justify-between gap-3 border-t border-slate-800 pt-6">
        <button type="button" onClick={() => onStepChange(Math.max(0, currentStep - 1))} disabled={currentStep === 0} className="inline-flex min-h-11 items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-400 transition hover:bg-slate-800 hover:text-white disabled:invisible sm:px-4"><FaArrowLeft />{t("resumeBuilder.actions.previous")}</button>
        {currentStep < steps.length - 1 && <button type="button" onClick={() => onStepChange(currentStep + 1)} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-sky-500 px-4 py-2.5 text-sm font-bold text-slate-950 transition hover:bg-sky-300 sm:px-5">{t("resumeBuilder.actions.next")}<FaArrowRight /></button>}
      </div>
    </div>
  );
};
